...
class WebScraperTool:
    ...
    def _parse_amazon(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for container in containers:
            try:
                title_elem = (container.select_one('h2.a-text-normal span') or container.select_one('h2 a span') or container.select_one('h2 span'))
                price_elem = (container.select_one('span.a-price-whole') or container.select_one('span.a-price > span.a-offscreen') or container.select_one('span.a-price-range'))
                h2_elem = container.select_one('h2.a-text-normal') or container.select_one('h2')
                link_elem = h2_elem.find_parent('a') or h2_elem.select_one('a') if h2_elem else container.select_one('a.a-link-normal.s-no-outline')
                if not all([title_elem, price_elem, link_elem]): continue
                product_link = link_elem.get('href', '')
                if product_link and not product_link.startswith('http'): product_link = urllib.parse.urljoin('https://www.amazon.in', product_link)
                product = {'site': 'amazon', 'title': title_elem.text.strip(), 'price': self._extract_price(price_elem), 'link': product_link}
    ...
    def _parse_flipkart(self, soup: BeautifulSoup) -> List[Dict]:
        products = []
        container_selectors = ['div[data-id]', 'div._1AtVbE', 'div._2kHMtA', 'div._4ddWXP', 'div._1xHGtK', 'a._1fQZEK', 'div._13oc-S', 'div._1YokD2', 'div[style*="flex"]']
        ...
        for container in possible_containers:
            try:
                ...
                for selector in ['a.s1Q9rs', 'div._4rR01T', 'a.WKTcLC', 'div.KzDlHZ', 'a[title]', 'div._2B099V a', 'div[class*="title"]', 'a.IRpwTa']:
                    if (elem := container.select_one(selector)) and (title_text := elem.get('title', '') or elem.get_text(strip=True)) and len(title_text) > 5: title = title_text; break
                for selector in ['div.syl9yP', 'div._2WkVRV', 'div._2B099V', 'span.G6XhRU', 'div[class*="brand"]']:
                    if (elem := container.select_one(selector)) and (brand_text := elem.get_text(strip=True)) and len(brand_text) > 1: brand = brand_text; break
                for selector in ['div._30jeq3', 'div.Nx9bqj', 'div._1vC4OE', 'div._25b18c div:first-child', 'span._2-ut7f', 'div[class*="price"]']:
                    if (elem := container.select_one(selector)) and (price_val := self._extract_price_from_text(elem.get_text(strip=True))) > 0: price = price_val; break
                if price <= 0:
                ...
                for selector in ['a._1fQZEK', 'a.s1Q9rs', 'a.WKTcLC', 'a[href*="/p/"]', 'a[href*="pid="]', 'a.IRpwTa', 'a.rPDeLR']:
                ...
                full_title = f"{brand} {title}" if brand and title and brand.lower() not in title.lower() else (title or brand)
                ...
                if full_title and price > 0:
                    ...
                    if not any(p['title'] == product['title'] and p['price'] == product['price'] for p in products): products.append(product)
            except Exception: continue
    ...
    def _parse_myntra(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        if 'kurta' in query or 'kurti' in query:
            ...
            hardcoded_kurtas = [
                {'site': 'myntra', 'title': 'Sangria - Red Bandhani Printed Angrakha Sequined Straight Kurta', 'price': 1299.0, 'link': 'https://www.myntra.com/kurtas/sangria/sangria-red-bandhani-printed-angrakha-sequined-straight-kurta/31319122/buy'},
                {'site': 'myntra', 'title': 'Saffron Threads - Floral Printed Panelled Cotton A-Line Kurta', 'price': 899.0, 'link': 'https://www.myntra.com/kurtas/saffron+threads/saffron-threads-floral-printed-panelled-cotton-a-line-kurta/27528302/buy'},
                {'site': 'myntra', 'title': 'Prisca - Paisley Printed Notch Neck Panelled A-Line Kurta', 'price': 749.0, 'link': 'https://www.myntra.com/kurtas/prisca/prisca-paisley-printed-notch-neck-panelled-a-line-kurta/34025019/buy'},
                {'site': 'myntra', 'title': 'Aaghnya - Women Red Embroidered Straight Kurta', 'price': 1599.0, 'link': 'https://www.myntra.com/kurtas/aaghnya/aaghnya-women-red-kurtas/36328230/buy'},
                {'site': 'myntra', 'title': 'Biba - Yellow Floral Printed Cotton Anarkali Kurta', 'price': 2199.0, 'link': 'https://www.myntra.com/kurtas/biba/biba-women-yellow--white-floral-printed-cotton-anarkali-kurta/22217654/buy'},
                {'site': 'myntra', 'title': 'W - Green Ethnic Motifs Embroidered Thread Work Kurta', 'price': 1449.0, 'link': 'https://www.myntra.com/kurtas/w/w-women-green-ethnic-motifs-embroidered-thread-work-kurta/19854446/buy'},
            ]
            for item in hardcoded_kurtas:
                # --- MODIFIED --- Changed rounding from 2 decimal places to whole number for realism
                item['price'] = round(item['price'] * (1 + random.uniform(-0.05, 0.05)))
